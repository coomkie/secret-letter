...
export class StatisticRepositoryImpl implements IStatisticsRepository {
  ...
  async getCommonStatistic(
    userId: string,
  ): Promise<GetCommonStatisticResponse> {
    ...
    const growRateReportsNotSolved = this.calcGrowthRate(
      reportsYesterday,
      reportsToday,
    );
    ...
    const growRateReportsSolved = this.calcGrowthRate(
      solvedYesterday,
      solvedToday,
    );
