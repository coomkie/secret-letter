...
export class MatchesRepositoryImpl implements IMatchesRepository {
    ...
    async createMatch(data: CreateMatchRequest): Promise<Matches> {
        ...
        const receiver = await this.usersRepo.findOne({where: {id: data.receiverId}});

        if (!sender) throw new Error(`Sender with id ${data.senderId} not found`);
        if (!receiver) throw new Error(`Receiver with id ${data.receiverId} not found`);
    ...
    async getAllMatchAdmin(req: GetAllMatchAdminRequest) {
        ...
        if (req.senderId) qb.andWhere('sender.id = :senderId', {senderId: req.senderId});
        if (req.receiverId) qb.andWhere('receiver.id = :receiverId', {receiverId: req.receiverId});
    ...
    async getAllMatch(userId: string, req: GetAllMatchUserRequest) {
        ...
        if (req.status) qb.andWhere('match.status = :status', {status: req.status});
