...
export class GetAllLetterSentUseCase {
    ...
    async execute(userId: string, req: GetAllLetterSentRequest): Promise<{
        items: LettersResponse[];
        total: number;
        page: number;
        pageSize: number;
    }> {
        const { items, total, page, pageSize } = await this.lettersRepository.getAllLetterSent(userId, req);
